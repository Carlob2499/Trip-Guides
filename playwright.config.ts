/* Playwright harness for the accessibility gate (tests/visual/a11y.spec.ts, CI-gated via
   .github/workflows/required-gate.yml on PRs and .github/workflows/a11y.yml on push-to-main)
   and the D5 gallery screenshot baselines (tests/visual/gallery-baselines.spec.ts — see its
   header for why pinning the GALLERY is the right screenshot suite where pinning product pages
   was the wrong one). The page-shadowing screenshot-diff suite that used to live here was removed
