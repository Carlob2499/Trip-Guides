...
const LIGHT_BG = "#f2ede5"; // R5 lifted Day ground — mirrors content.config.ts, which mirrors base.css --bg
const DARK_BG = "#0f1b2e";
...
// researchFloors (D3) was DELETED with Pipeline V2 (2026-08-17): fixed candidate quotas were
// replaced by the adaptive saturation record (DECISIONS.md "Research breadth"). No shipped
// guide ever declared the field, so nothing migrates; a guide still declaring it is simply
// carrying an unknown key the schema strips. The funnel invariant (shipped ⊆ shortlist)
// lives on in check-candidates.mjs and its suite.
