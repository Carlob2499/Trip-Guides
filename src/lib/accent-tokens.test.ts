// @protects-file Every guide's colour scheme keeps its text readable against its own background.
...
const LIGHT_SURFACES = ["#ffffff", "#ebe6de", "#f5f1ea"];
const DARK_SURFACES = ["#232120", "#1b1917", "#121110"];
...
describe("accentTokens", () => {
  ...
  it("matches the defaults baked into base.css for the no-guide accent", () => {
    ...
    // accentTokens(--accent). Hand-copied values drift; this is the thing that notices.
    // 2026-09-07: --accent moved #d35c16 -> #c2410c with the Warm Stone palette, so the source
    // this derives from moves too. The assertion is unchanged — it still says "base.css must
    // equal the function's live output" — only the accent it is asked about.
    const css = readFileSync("src/styles/base.css", "utf8");
    const t = accentTokens("#c2410c");
