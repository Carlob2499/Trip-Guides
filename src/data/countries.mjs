...
//
// ⚠ Exchange RATES are perishable and are never authored by hand here. The live
// Frankfurter service (see GuideLayout) is the source of truth at runtime; FALLBACK_RATES
// below is a GENERATED seed table (scripts/refresh-fx.mjs, stamped with its fetch date)
// shown only until the live rate loads, or when it can't be reached.
// It is not optional decoration: rate.js returns early for any currency with no entry, so
// a missing rate silently removes the whole exchange-rate feature from that guide rather
// than degrading it. That is why the table now covers every currency Frankfurter publishes
// instead of the four that happened to be hardcoded first — and why the ones it does NOT
// publish are left absent rather than guessed.
...
export const FALLBACK_RATES_AS_OF = "2026-07-31";
export const FALLBACK_RATES = {
  AUD: 1.425, BRL: 5.058, CAD: 1.404, CHF: 0.81, CNY: 6.751, CZK: 21.1,
  DKK: 6.509, EUR: 0.871, GBP: 0.745, HKD: 7.843, HUF: 317, IDR: 18052,
  ILS: 3.057, INR: 95.4, ISK: 124, JPY: 160, KRW: 1444, MXN: 17.4,
  MYR: 4.087, NOK: 9.527, NZD: 1.706, PHP: 61.3, PLN: 3.756, SEK: 9.565,
  SGD: 1.285, THB: 33.5, TRY: 47.5, USD: 1, ZAR: 16.6,
};
...
// ISO 3166-1 NUMERIC — a small, deliberately incomplete map (docs/archive/INDEX.md → PLAN_ATLAS_MIGRATION Stage
