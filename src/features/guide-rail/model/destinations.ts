...
export interface ProjectedSection<T extends SectionLike = SectionLike> {
  /** The original object, deliberately not cloned or rewritten. */
  section: T;
  /** Its position in the original sections array. */
  index: number;
  rawGroup: string;
...
/**
 * Return one canonical home for a section. Combined food-and-shopping groups are decided at
 * section level so their shopping-only material can move to Explore without rewriting data.
 */
...
/** Stable public route name for a canonical section owner. */
...
/** Project sections without cloning, dropping, or changing their original indices. */
...
/**
 * Compatibility aliases for the pre-R6 public `#grp-N` contract. Raw groups used their first
 * authored section as the group landing point; appended recap/tools indices followed that raw
 * group list. The aliases live inside their named R6 panel so the generic hash resolver can
 * keep locating the closest `.catblock` without knowing either navigation generation.
 */
