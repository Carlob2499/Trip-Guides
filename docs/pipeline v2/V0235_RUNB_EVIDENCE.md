...
- **Deterministic pipeline (new, for the repair list):** (R-C) the reconcile gate accepted `b153af3` although its tree fails `npm run build` (facts.json `≈`+`state:"approx"` chip renders into two panels lacking `verified_on`) — the reconcile gate does not run the build gate the critic runs; (R-D) the `check-candidates.mjs` shipped-name cross-check flagged "Camel commute" — a descriptive Pass-B candidate name, not a proper noun the guide would carry — as not-in-guide at critic a3 (correction on independent review: this finding was a3's *output* beside two independent failing categories and consumed no budget; it is the third distinct name-contract mismatch class after the canary's qualifier case, already fixed and regression-pinned, and Tokyo's markdown-bold case, repaired producer-side in-run — the repair belongs in the ledger name contract, not in loosening the anti-padding matcher); (R-E) **dual-pass corroboration counted two passes converging on the same wrong number (the 600 m viewpoint distance) as independence** — the most cutover-relevant structural finding; (R-F) `coverage.v2.json` over-claims: every ask "covered"/reason null in both states, including a priority the HEAD guide itself declares unresearched, with disproven evidence ids backing the BINDING-mobility ask. Also recurring from Run A: (R-A) post-critic `evidence.v2.json` desync (byte-identical at both states, contradicting the HEAD guide on the anchor transfer — the artifact V07 reads); (R-B) usage-limit failures consume the bounded attempt budget.
