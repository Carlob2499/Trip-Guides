...
- **Days until September 30 engineering complete:** 32
- **Highest-risk unfinished items:** one clean fresh post-remediation release-readiness run must still prove Reconcile → Critic → landing on the frozen rebuilt Kumamoto candidate; final protected-main repository settings still require owner/tooling capability; and frontend finalization must be pulled forward before the Sep 20/Sep 27 freezes without destabilizing the frozen acceptance candidate.
