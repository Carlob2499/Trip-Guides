...
- **Post-Fukuoka remediation candidate:** PR #117 is a new hash-pinned model-input candidate, not a continuation of Fukuoka. It makes retained validator findings repair-first, forbids search-preview relabeling as verification, preserves unaffected work, and removes Reconcile's impossible shell/offline-gate instruction. No Claude/Fable run is part of this remediation.
...
| A02 | **Post-Fukuoka model-input remediation candidate** | Acceptance repair | Codex + Automated CI/test | Aug 29 | **DONE / YELLOW — DETERMINISTIC REMEDIATION; LIVE ACCEPTANCE PENDING** | A01 | PR #117; repair-first prompt/feedback regressions; historical Fukuoka and new candidate blob hashes pinned separately | Fresh Reconcile → Critic → landing proof still absent | Future canary only after candidate is merged/validated and Claude allowance is deliberately available |
