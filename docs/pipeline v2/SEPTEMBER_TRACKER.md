...
- **Highest-risk unfinished item:** one clean fresh post-remediation release-readiness run must still prove Reconcile → Critic → landing on the frozen rebuilt Kumamoto candidate. All currently known deterministic preflight gates are green; the remaining backend uncertainty is live model/output behavior, not an identified code defect.
