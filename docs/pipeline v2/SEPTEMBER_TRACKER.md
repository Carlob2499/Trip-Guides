...
- **Current phase:** Fukuoka remains the failed historical final-acceptance attempt, but its post-failure deterministic/model-input remediation is now merged. A **fresh Kumamoto release-readiness acceptance candidate is PRE-FLIGHT GREEN** on frozen head `99d758756a2f4a53fef5f3072d72388da4ebdb17`, based on accepted main `82d8d78a6411c2d8c944087df598ccf03afec940`. No model-backed Kumamoto workflow has been dispatched yet.
...
- **Kumamoto preflight:** closed-unmerged PR #120 proved the exact frozen candidate head with Tests PASS, Accessibility PASS, CodeQL actions PASS, and CodeQL javascript-typescript PASS. The earlier preflight PR #118 is preserved as the deterministic viewport failure that led to merged PR #119; the candidate was then rebuilt cleanly from corrected main.
...
- **Preservation rule:** do not repair-and-merge `research-v2/fukuoka` or continue it past its cap. Do not mutate the frozen Kumamoto candidate after its green preflight; any deterministic defect discovered during the new run invalidates that run and requires a separately repaired/rebuilt candidate.
- **UI/design isolation:** PR #115 is closed unmerged and deferred. Its traveler-first IA history is preserved for post-backend extraction/rebuild; it is not part of the acceptance candidate.
...
- **Highest-risk unfinished item:** one clean fresh post-remediation release-readiness run must still prove Reconcile → Critic → landing on the frozen Kumamoto candidate. All currently known deterministic preflight gates are green; the remaining backend uncertainty is live model/output behavior, not an identified code defect.
...
| A03 | **Fresh post-remediation release-readiness acceptance — Kumamoto** | Acceptance | Claude/Fable + Codex | Sep 2 | **READY FOR REVIEW — PREFLIGHT GREEN; MODEL DISPATCH PENDING** | A02 | Frozen branch `acceptance/v2-kumamoto-20260902` at `99d758756a2f4a53fef5f3072d72388da4ebdb17`; closed PR #120 exact-head Tests/A11y/CodeQL all PASS; criteria in `FINAL_V2_ACCEPTANCE_KUMAMOTO.md` | Live model-backed Reconcile → Critic → landing proof not yet executed | Dispatch exactly once from frozen ref; no cap extension, artifact hand-edit, cutover, publication, or candidate mutation |
