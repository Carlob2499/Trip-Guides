...
The validation runs answered the question they were designed to answer, and the answer is asymmetric: **detection is proven, repair-to-green is unproven.** In both runs the pipeline's own fresh-context critic caught every planted-class defect — including all three immediate-FAIL facts — and the deterministic gates refused every inconsistent state. But neither run reached a state that is simultaneously gate-accepted and self-consistent, and Run B shipped (at its accepted checkpoint) three traveler-harmful facts that only died because the budget-capped critic stage existed. Cutover before the repair list below would put production authority on a path whose containment works but whose completion has never once been observed.
...
5. **R-B — budget accounting:** usage-limit interruptions (an availability loss, 3 attempts across the program) should not consume the same bounded quality-retry budget as real failures — both runs stopped one gate short for exactly this reason.
