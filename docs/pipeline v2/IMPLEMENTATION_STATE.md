...
The trusted `/new` caller pins those four production defaults explicitly instead of merely inheriting reusable-workflow defaults. The Chat/OpenAI role equivalents used for independent testing are Terra Medium for evidence gathering and Sol Medium for reconciliation/criticism; they are analogues only and are never passed to the Claude Code workflow. High effort is an explicit escalation for genuinely consequential judgment, not a fresh-run default.
...
**Kumamoto preflight:** r1/r2/r3 remain preserved historical preflight evidence only. r3 `acceptance/v2-kumamoto-20260902-r3` at `56e513000792bc71bf4e18c0a0909724fe5cebac` was exact-head GREEN on accepted base `57e320535d1cb6e861a5001f8c26cc718dcfd93d` (closed-unmerged PR #167), but later acceptance-sensitive V2/control-plane changes — including role-based model routing and the final traveler-facing prose contract — invalidate that candidate for dispatch. No model-backed Kumamoto workflow has been dispatched. The next acceptance candidate must rebuild/replay the same pre-registered Kumamoto scenario from settled current `main`, then receive a fresh exact-head deterministic proof and drift audit before any model spend.
...
**Next engineering surface:** land the settled V2 model-routing + prose-contract revision, then rebuild/replay the pre-registered Kumamoto scenario from that settled `main`. Prove the new candidate's exact head deterministically and perform the fresh control-plane drift/model-burn firewall audit immediately before the single model-backed acceptance dispatch. Do not dispatch r1/r2/r3, continue Fukuoka, extend quality/auto-retry caps, hand-edit model-owned output, change the production selector, publish, or merge the canary. V1 remains the production default and rollback path until that fresh acceptance passes and production cutover is explicitly approved.
