...
| 6 | **Phrase cards + speak** ✅ | Verified situational cards (allergy/taxi/help), native script big-type + romanization, free browser TTS (Web Speech API, zero cost) | Guide-level `phrases` field (NOT a section type — deliberately sidesteps the bucket()/tabBudget system, same reasoning as `cover`), rendered in **Trip kit** | **SHIPPED — and now POPULATED** (Korea + Denmark carry 5 verified phrase cards each; Japan and Sedona carry none, which is the honest blank, not a gap). It shipped dormant on purpose — a wrong native-script phrase is safety-adjacent, so nothing was fabricated ahead of research. Speak button feature-detects `speechSynthesis`, degrades to silent (phrase text alone is still fully useful). |
| 7 | **Entry-requirements card** ✅ | Visa/ETA + passport-validity per traveler home country — pipeline-researched, source+date stamped, recert-refreshed | Guide-level `entry` field (same reasoning as `phrases`); rendered in **Trip kit**, first card (settle before you book) | **SHIPPED — and now POPULATED** on Korea, Denmark and Japan (one researched `entry` row each; Sedona has none — a US-domestic trip needs no entry card). `source_url`+`verified_on` are SCHEMA-REQUIRED, stronger than the general provenance pattern, because a wrong visa claim can mean a denied boarding. NEVER a paid visa API. |
...
| 9 | **Advisory pill** ✅ | Official travel-advisory level + primary link, shown ONLY when elevated (honest-blank doctrine) | SOS silo extension + hub badge; source fetched at build/recert — **verify the official free source before building** | **SHIPPED.** Source: US State Department (`travel.state.gov`) per-country advisory pages — free, official, no key. **Not build-time-fetched** (the page is Cloudflare-gated against plain `fetch`/`curl`/WebFetch — confirmed live; only resolves through an actual browser); researched like `entry`, guide-level `advisory` field, `source_url`+`verified_on` required. Real, verified content shipped on both guides: Korea = Level 1 (checked, not elevated — pill silent by design); Denmark = Level 2 "Exercise Increased Caution" (terrorism) — pill live on the hub card + SOS sheet, both verified in-browser. |
| 10 | **Trip recap card** ✅ | Post-trip shareable image (days, done-vs-skipped from learnings, group spend) via the existing sharp OG generator | OG-generator reuse + learnings silo | **SHIPPED.** `src/pages/recap/[slug].png.ts` — same sharp/SVG technique as `og/[slug].png.ts`, dark "after the trip" ground so it never reads as a come-plan-this card. `tripRecapStats()` (`src/features/exports/model/exports.ts`) computes `hit = waypointsTotal − skippedTotal` — verified this exact formula reproduces Korea's own hand-written "21 of 37 planned stops" narrative from its real data (regression-tested). Only generated for guides with a `learnings` block (honest-blank — no post-trip reflection, no recap route at all). "Share your trip recap" link added to the Learnings tab foot. |
...
## Held → since SHIPPED (both, after this table was written)

| # | Feature | Why it was proposed | Where it landed |
|---|---------|---------------------|-----------------|
| 2 | **Prep timeline (T-minus deadlines on the booking checklist)** | "Coordination failure kills group trips" — the strongest discourse finding. Additive `due` field; zero API. | **SHIPPED.** The additive `{ text, due? }` checklist upgrade (`src/lib/checklist.ts`), bucketed by `src/features/trip-kit/model/book-by.ts`, rendered in `TripKit.astro`. |
| 3 | **Budget pact + actual-vs-plan** | 61% overspend from social pressure; intake budget vs Trip Split live actuals. Zero API. | **SHIPPED.** `src/features/budget-pact/` (full silo: `model/pact.ts`, `ui/pact.js`, `styles.css`), booted from `src/scripts/guide-ui.js`. |

*(Nothing is currently held. A feature lands here only while it's a live option — once it
ships or is dropped it moves to the table above or to Dropped, so this section reads as a
decision queue rather than a graveyard.)*
