...
| D13 | **`.resume-chip` carried a second press depth** | Visual | `touch.css` pressed it at `.98` on the transform channel. It is a `<button>`, so `base.css` already covered it — the two would have composed to `.941` and quietly reintroduced D2 the day the component ships | The `:active` rule and its reduced-motion mirror are gone; the component's own styles are untouched, because deleting a styled surface belongs to whoever owns the feature |
...
**D8 — accepted, provisionally.** "Fine for now; should be re-evaluated with design changes."
No baseline work was needed: the `Gallery screenshot baselines` step in `required-gate` ran and
**passed on CI with the edge in place** (run 34294849688), so a 1px outline at 10% falls inside
the comparison threshold on CI's Chromium. The caution about regenerating baselines was right in
principle and moot in fact.

**Lane B — accepted, provisionally.** Every "Fidelity — open" item in `board-vs-build.md` is
accepted as built, and that file now records what the acceptance costs: `VISUAL_LINEAGE.md` makes
the board mandatory ancestry, so the build's composition is the thing to preserve on those
surfaces until someone decides otherwise. Re-open at the next design change.
