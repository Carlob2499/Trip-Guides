...
| 8 | Priority depth | Personal | the intake's top-2–3 ranked priorities get real depth; low-ranked ones are light or cut — never "optimize for everything". **Adaptive since Pipeline V2 (2026-08-17, supersedes the S2/S3 numeric floors):** breadth scales to the destination; the run's saturation record must justify the stop (duplicates/weaker trend + unresolved evidence unlikely to change the recommendation), the `## Candidates considered` tables may not be empty, and every `shipped` name must exist in the guide (shipped ⊆ shortlist) | **verify (candidates row)** + V2 saturation gate + human | **P1** |
