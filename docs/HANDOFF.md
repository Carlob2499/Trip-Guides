...
direct `src` yet — the capability is live but unexercised.** And session #30b's repository-breadth
pass is exactly where it earns out: Oslo, Sapporo, Sendai, Daejeon, Busan and Sedona all gained
sights this session, and any of those with no honest Commons photo is now sourceable instead of
shipping bare. Merged to `main`."
