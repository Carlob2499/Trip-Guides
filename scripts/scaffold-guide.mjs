...
//   2. guides-intake/<slug>/            — the guide's run-state directory: `intake.md` (the
//      answers, frozen traveler intent), `ledger.md` (the empty research-state skeleton a later
//      Claude Code research pass fills), `state.json` (the checkpoint spine) and `coverage.json`.
//
...
// Fill docs/standards/new-guide-intake.md with what the quick-start collected; leave the rest
// blank. TRAVELER INTENT ONLY — the research state this used to carry now scaffolds separately
// into ledger.md (buildLedgerMd below), so the intent half can be frozen after scaffold while
// the research half is appended to for the whole run.
...
// The research-state half of the run-state directory, scaffolded empty beside intake.md. THIS
// FUNCTION IS THE SKELETON'S ONLY HOME — docs/standards/new-guide-intake.md documents the intent
// template and points here rather than repeating these headings, so the two can't drift.
// Append-only sections a later stage adds (verification ledger, critic findings, citation audit,
// continuity sweeps) are deliberately NOT stubbed: an empty heading nobody wrote is noise, and
// each of those stages appends its own.
export function buildLedgerMd(answers = {}) {
  return `# Research ledger — ${answers.country || "[Destination]"}
...
// Pick a slug no guide directory already occupies.
async function uniqueSlug(base, guidesDir = GUIDES_DIR) {
  let slug = base, n = 2;
  while (existsSync(path.join(guidesDir, slug))) {
    slug = `${base}-${n++}`;
  }
  return slug;
}

// Write the guide directory + the run-state directory for a set of answers.
// Returns { slug, guidePath, intakeDir, intakePath, ledgerPath, coveragePath, statePath }.
// The dir overrides exist so a test can exercise the real emitted LAYOUT hermetically, the
// same way writeGuideDir already takes `guidesDir`.
export async function writeScaffold(answers, { guidesDir = GUIDES_DIR, intakeDir = INTAKE_DIR } = {}) {
  ...
  const slug = await uniqueSlug(slugify(a.slug || a.country || a.title), guidesDir);
  const guide = buildGuideObject(a);
  await mkdir(guidesDir, { recursive: true });
  // One directory per guide holds the whole run: frozen intent, research ledger, checkpoint
  // state and the coverage matrix — so nothing about a run is a loose sibling file any more.
  const runDir = path.join(intakeDir, slug);
  await mkdir(runDir, { recursive: true });
  const intakePath = path.join(runDir, "intake.md");
  const ledgerPath = path.join(runDir, "ledger.md");
  await writeFile(intakePath, buildIntakeMd(a));
  await writeFile(ledgerPath, buildLedgerMd(a));
  /* Every guide is a DIRECTORY — one uniform shape across the whole catalog, drafts included.
     writeGuideDir is reused rather than re-deriving the NN-<group>.json ordering here: that
     naming decision lives in exactly one place (scripts/lib/guide-shape.mjs). */
  await writeGuideDir(slug, guide, { guidesDir });
  const guidePath = path.join(guidesDir, slug);
  ...
  await initState(slug, { intakeDir });
  return { slug, guidePath, intakeDir: runDir, intakePath, ledgerPath, coveragePath, statePath: path.join(runDir, "state.json") };
}
...
async function main() {
  ...
  console.log(`[scaffold] wrote ${path.relative(ROOT, res.guidePath)} + ${path.relative(ROOT, res.intakeDir)}/ (slug: ${res.slug})`);
}
