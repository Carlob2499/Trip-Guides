  ...
  for (const e of entries) {
    try {
      if (e.isFile() && e.name.endsWith(".json")) {
        ...
        let guide = { ...JSON.parse(metaRaw), sections };
        // The fact registry, applied the same way the Astro loader applies it — so every
        // checker downstream (voice grep, completeness, the undated-price advisory, staleness)
        // reads the text a traveler will actually see. Skipping this would silently defuse
