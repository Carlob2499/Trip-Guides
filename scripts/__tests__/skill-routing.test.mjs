...
const CURRENT_VALIDATION_CANDIDATE_BLOBS = {
  // Headless V2 prompts read the .agents tree. Pin the COMPLETE effective Guide-Author input
  // stack, not merely the canonical wrapper: a reference-file change can alter research/prose
  // behavior just as materially as a prompt change. The separate parity suite proves .claude
  ...
  /* Repinned 2026-09-06 under the explicit review this fence exists to force. The change:
     "genuinely" and "truly" join the anti-generative list (17 shipped uses audited, every one
     deletable without loss), and a paragraph records that contractions and reference-shape
     em-dashes are a register choice rather than a ban. It is a deliberate voice-standard change
     to the Guide-Author input stack, reviewed as such, not prompt drift. Parity with the .claude
     copy was verified before repinning. */
  ".agents/skills/waypoint-guide-author/references/block-types.md": "af787f367e0cc75816106142bbd6142a5ff325ec",
