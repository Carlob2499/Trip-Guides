...
- **Astro / CSS / UI / layout:** use `waypoint-design` plus affected source files. Presentation work must not alter factual content. `docs/reference/design-system.md` is the single design authority; load `motion.md` / component registry as needed. Historical handoffs/prototypes are consult-only and must not override current authority.
