...
| Daichan (Gion Daichan) | shipped — a yatai specializing in udon | y |
...
| Hakata Yatai Yokocho Kizuimaru (Daimyo) | shipped — Pass B find; indoor group-yatai venue directly solving the party-of-8 seating problem; ⚠ unresolved renovation-closure note | y |
...
| Motsunabe Kobayashi (Imaizumi) | shipped — Pass B find; promoted to the Day 2 dinner anchor for its 8+-person group course + online booking | y |
...
| Fukuya | shipped — Pass B independently reaches Pass A's own shipped pick; AGREE | y |
...
- **2026-08-29 (Reconcile, retry) — the prior attempt's guessed gaps closed against the real validator's actual findings.** A live run of the deterministic gates (fed back verbatim, not re-derived) surfaced 8 blocking `check-research.mjs` findings, 3 blocking `check-candidates.mjs` findings, 1 `check-voice` leak, and 1 `check-uncertainty.mjs` `regulatory-change-unregistered` finding — script execution was still blocked this attempt too (same permission gate as above), so each fix was made by reading the owning checker's source directly (`scripts/audit/check-research.mjs`, `scripts/check-candidates.mjs`, `scripts/verify-guide.mjs`'s `checkVoice`, `scripts/audit/check-uncertainty.mjs`) and hand-tracing its exact logic against the artifact, the same discipline as the paragraph above. Fixes: (1) 3 of 4 `05-transit.json` map-point `place_id`s resolved to real OSM ids by querying Nominatim directly with `lookup-place.mjs`'s own parameters (Fukuoka Airport `W529445584`, Hakata Station `W72653571`, Sakurai Futamigaura `N6386259585`, name-matched against the guide's existing coordinates) — Dazaifu Tenmangu's shrine complex itself has no clean single-entity Nominatim match (searches only surface the Information Center and Treasure Hall as separate POIs), so its `place_id` stays the honest `__VERIFICATION_REQUIRED__` placeholder rather than attaching the wrong entity. (2) Item-level `source_url`/`verified_on`/`shelf_life` added to the one `budget` item and 3 `days` items `check-research.mjs` flagged as carrying an undated hour/price figure under this guide's `provenance:"strict"` (Days 1/3/5; Day 2 and Day 4 were already clean). (3) `## Candidates considered`: "Daichan / Gion Daichan (yatai udon)" renamed to the canonical `Daichan (Gion Daichan)` (matches `evidence.v2.json`'s candidate name after the checker's trailing-parenthetical strip); the "*(Pass B)* Fukuya" row's leading annotation moved out of the Candidate cell into the Verdict cell (a leading, non-trailing parenthetical/prefix isn't stripped by the name-normalizer, so it silently failed the canonical-name match); `06-days.json`'s Day 4 body changed "Itoshima's oyster huts" → "the Itoshima oyster huts" so the exact candidate-name substring the checker requires is actually present (the possessive apostrophe broke the contiguous-substring check even though the candidate itself was mentioned). While fixing these, two more rows were found silently exempted from the same checks by a **markdown-bold** verdict cell (`**shipped**` doesn't match the checker's `/^shipped\b/i`, so Kizuimaru and Motsunabe Kobayashi's shipped rows were never actually being validated) — both de-bolded and had their own leading "*(Pass B)*" prefix moved to the verdict text for the same reason as Fukuya, so all three now validate for real rather than being accidentally skipped; Komyozenji's `**detour**` cell was de-bolded too for consistency (verdict `detour` isn't checked by `check-candidates.mjs` at all, so this was cosmetic only). (4) The one process-language leak `check-voice` flags (`crowd_tip` on `08-food-and-shopping.json`'s Motsunabe Kobayashi entry — "weren't stated on the operator's own site **this pass**") reworded to drop the process phrase without losing the fact; the same phrase in `06-days.json`'s Day 2 body was fixed too even though `checkVoice` doesn't scan day-item `body` fields (only section-level `body`/`intro`/`why`/`crowd_tip` and item-level `why`/`crowd_tip`), for consistency. (5) `facts.json` (previously `{}`) gained a `money-currency-tax-free-cutover` row (source: `japan.travel`, the same source the guide's own Money & currency panel already cites) whose `value` contains "Nov 1, 2026" — `regulatoryChangeUnregistered` matches the panel's "switches to a refund-at-departure model on Nov 1, 2026" against `facts.json` text and previously found no row covering that cutover date. `leg-duration-unverified` / `routes-not-configured` (also in the fed-back findings) were left as-is: both come from `checkRoutes`, which is explicitly advisory-always by design (`GOOGLE_ROUTES_KEY` unset, and every guide in the corpus carries some unsourced `≈` leg durations) and never fails a run. `coverage.v2.json` was re-checked against the current per-file anchor list and needed no changes — every `where` ref and required ask id already resolves correctly.
