...
guide-content standards. `AGENTS.md` auto-loads and binds the universal principles (the four
...
- **Edit an existing guide** — verify the new/changed fact per the rules (update its verification
  date as written), then run the continuity sweep from AGENTS.md's **"Editing a Guide —
...
**AGENTS.md's Ship Loop governs every change and is not optional here** — build, `npm test`,
