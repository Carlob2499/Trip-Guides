/* The plate line, on the page (R5 SCREENS §1 + SUPERSEDES §3). plate-line.ts's own tests pin
   ...
   Every navigation here waits for `domcontentloaded`, not `networkidle`. This is the one spec
   that freezes the page clock, and both tests that flaked under full-suite parallel load — and
   passed alone — were navigations made after that freeze. networkidle wants 500ms of network
   quiet, which is the most contention-sensitive wait there is when eight workers share one
   preview server. Every assertion below auto-retries against a locator, so readiness is
   asserted by the thing being asserted. */
...
test("⌁ the coordinate pair and the plate number are gone from the guide", async ({ page }) => {
  // Asserted as absence in the BUILT page, not by reading the layout — a still-matching rule would keep them alive invisibly.
  await page.goto(KOREA, { waitUntil: "domcontentloaded" });
...
test("⌁ the eyebrow and the plate line never say the same thing", async ({ page }) => {
  // The kicker holds both halves; rendering it whole in both places would show the cities twice, as if two separate facts.
  await page.goto(KOREA, { waitUntil: "domcontentloaded" });
  /* Retrying, not a bare innerText(): gsap-hero.js fades the eyebrow in from autoAlpha:0 and
     innerText reads "" while it is visibility:hidden — networkidle used to outlast that tween
     by accident. useInnerText applies the CSS uppercasing; matched case-insensitively. */
  const eyebrowEl = page.locator(".mast-eyebrow");
  await expect(eyebrowEl).toHaveText(/^jul 8–15, 2026$/i, { useInnerText: true });
  const eyebrow = (await eyebrowEl.innerText()).trim().toLowerCase();
...
test("the next leg appears mid-trip, naming a real stop from the day cards", async ({ page }) => {
  ...
  // textContent, not innerText: the row is uppercased by CSS and the assertion is about the string the page composed, not the casing it paints.
...
test("the live-state column stamps the trip's state, and the stamp follows the clock", async ({ page }) => {
  /* Three states, one page, three different days. See the file header on `domcontentloaded`. */
...
test("⌁ a guide with nothing to count renders no progress row", async ({ page }) => {
  // Japan carries no day waypoints, so its row is the booking half alone — never "0 stops", which would read as a researched answer of zero.
  await page.goto("/Trip-Guides/guides/japan/", { waitUntil: "domcontentloaded" });
...
test("⌁ a plate line with nothing to say is shorter, never padded", async ({ page }) => {
  // Every row is independently conditional. Walked across all four guides: whatever each renders, no row is ever an empty box or a placeholder dash.
