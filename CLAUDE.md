...
10. **Never construct a URL from inference — describe the navigation path instead.** If a URL cannot be opened and confirmed to resolve in this session, give step-by-step navigation ("Settings → Pages → GitHub Actions") rather than a clickable link. Constructing a URL from git remote output, repo names, or training knowledge and presenting it as verified is the same class of error as a hallucinated fact.
11. **Zero grep results is not proof of absence — verify the grep itself.** When a grep returns 0 results for something that should exist (e.g. "no absolute hrefs found"), check that the pattern actually matches before concluding nothing is there. Regex lookaheads, encoding mismatches, and shell quoting silently produce false negatives. Cross-check with a simpler pattern or read the file directly.
12. **When the Edit tool fails to match once on a line with unicode or special punctuation, switch to a Node.js script immediately.** Em dashes (U+2014), non-breaking spaces (U+00A0), and curly quotes look identical in text but break string matching. Do not retry the Edit tool — write a `.js` script to the scratchpad and run it with `node`.
13. **After any `.github/workflows/` change, confirm a green Actions run before closing the task.** Push → open the Actions tab → wait for the run to complete. A workflow file that parses correctly is not the same as a workflow that runs correctly. The Node.js version bug was caught only by the user, hours after push.
14. **When adding or changing a base path or deploy target, audit ALL internal `href` and `src` attributes before the first push.** Run: `grep -rnE 'href=|src=' src/ | grep -v 'https\?://' | grep -E '["'"'"'`]/` to find root-absolute internal links, then verify each appears correctly in `dist/`. This catches the entire class of base-path 404 bugs in under 2 minutes.
